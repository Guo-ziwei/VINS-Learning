...
System::System(string sConfig_file_) : bStart_backend(true) {
    ...
    ofs_pose.open("./pose_output_MH_05_DogLeg.txt", fstream::out);
    if (!ofs_pose.is_open()) {
...
void System::PubImageData(
    double dStampSec, std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>>& cam_points,
    std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>>& features) {
    if (!init_feature) {
        cout << "1 PubImageData skip the first detected feature, which doesn't contain optical flow speed" << endl;
        cout << "1 image timestamp: " << dStampSec << endl;
        init_feature = 1;
        return;
    }

    if (first_image_flag) {
        cout << "2 image timestamp: " << dStampSec << endl;
    ...
    if (PUB_THIS_FRAME) {
        ...
        // cout<<"image timestamp: "<<dStampSec<<endl;
        vector<set<int>> hash_id(NUM_OF_CAM);
        for (int i = 0; i < NUM_OF_CAM; ++i) {
            for (size_t j = 0; j < cam_points.size(); ++j) {
                int p_id = cam_points[j](3);
                double x = features[j].x();
                double y = features[j].y();
                x = 460.0 * x + 255.0;
                y = 460.0 * y + 255.0;
                // double z = 1;
                feature_points->points.push_back(Eigen::Vector3d(features[j].x(), features[j].y(), 1.0));
                feature_points->id_of_point.push_back(p_id * NUM_OF_CAM + j);
...
vector<pair<vector<ImuConstPtr>, ImgConstPtr>> System::getMeasurements() {
    ...
    while (true) {
        ...
        // cout<<"imu timestamp in measurement: "<<imu_buf.back()->header<<endl;
        // cout<<"Image timestamp in measurement: "<<feature_buf.front()->header<<endl;
        if (!(imu_buf.back()->header > feature_buf.front()->header + estimator.td)) {
            cerr << "wait for imu, only should happen at the beginning sum_of_wait: " << sum_of_wait << endl;
            sum_of_wait++;
            return measurements;
        }
...
void System::ProcessBackEnd() {
    ...
    while (bStart_backend) {
        ...
        if (measurements.size() > 1) {
            cout << "1 getMeasurements size: " << measurements.size() << " imu sizes: " << measurements[0].first.size()
                 << " feature_buf size: " << feature_buf.size() << " imu_buf size: " << imu_buf.size() << endl;
        }
        lk.unlock();
        m_estimator.lock();
...
void System::Draw() {
    ...
    while (pangolin::ShouldQuit() == false) {
        ...
        for (int i = 0; i < static_cast<int>(vPath_to_draw.size() - 1); ++i) {
            pangolin::glDrawLine(
                vPath_to_draw[i].x(), vPath_to_draw[i].y(), vPath_to_draw[i].z(), vPath_to_draw[i + 1].x(),
                vPath_to_draw[i + 1].y(), vPath_to_draw[i + 1].z());
        }
        // glEnd();
