...
void EdgeReprojection::ComputeResidual() {
    ...
    //    residual_ = information_ * residual_;   // remove information here, we multi information matrix in problem
    //    solver
}
...
void EdgeReprojectionXYZ::ComputeJacobians() {
    ...
    const Mat33&& ric = qic.toRotationMatrix();
    Mat23 reduce(2, 3);
    reduce << 1. / dep_i, 0, -pts_camera_i(0) / (dep_i * dep_i), 0, 1. / dep_i, -pts_camera_i(1) / (dep_i * dep_i);
