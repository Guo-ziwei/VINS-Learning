...
bool Chessboard::findChessboardCornersImproved(
    const cv::Mat& image, const cv::Size& patternSize, std::vector<cv::Point2f>& corners, int flags) {
    /************************************************************************************\
    ...
    for (int k = 0; k < 6; ++k) {
        for (int dilations = minDilations; dilations <= maxDilations; ++dilations) {
            ...
            if (flags & cv::CALIB_CB_ADAPTIVE_THRESH) {
                int blockSize =
                    lround(
                        prevSqrSize == 0 ? std::min(img.cols, img.rows) * (k % 2 == 0 ? 0.2 : 0.1) : prevSqrSize * 2) |
                    1;

                // convert to binary
                cv::adaptiveThreshold(
                    img, thresh_img, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::ThresholdTypes::THRESH_BINARY, blockSize,
                    (k / 2) * 5);
            } else {
                ...
                cv::threshold(img, thresh_img, thresh_level, 255, cv::ThresholdTypes::THRESH_BINARY);
            }
            ...
            cv::Mat kernel1 = cv::getStructuringElement(cv::MorphShapes::MORPH_CROSS, cv::Size(3, 3), cv::Point(1, 1));
            cv::Mat kernel2 = cv::getStructuringElement(cv::MorphShapes::MORPH_RECT, cv::Size(3, 3), cv::Point(1, 1));
            ...
            cv::rectangle(
                thresh_img, cv::Point(0, 0), cv::Point(thresh_img.cols - 1, thresh_img.rows - 1), CV_RGB(255, 255, 255),
                3, 8);
...
//===========================================================================
void Chessboard::findConnectedQuads(
    std::vector<ChessboardQuadPtr>& quads, std::vector<ChessboardQuadPtr>& group, int group_idx, int dilation) {
    ChessboardQuadPtr q;
...
void Chessboard::generateQuads(
    ...
    for (size_t i = 0; i < contours.size(); ++i) {
        ...
        if (approxContour.size() == 4 && cv::isContourConvex(approxContour)) {
            ...
            if (!(flags & cv::CALIB_CB_FILTER_QUADS) || (d3 * 4 > d4 && d4 * 4 > d3 && d3 * d4 < area * 1.5 &&
...
bool Chessboard::checkQuadGroup(
    std::vector<ChessboardQuadPtr>& quads, std::vector<ChessboardCornerPtr>& corners, cv::Size patternSize) {
    // Initialize
    bool flagRow = false;
...
void Chessboard::getQuadrangleHypotheses(
    const std::vector<std::vector<cv::Point> >& contours, std::vector<std::pair<float, int> >& quads,
    int classId) const {
...
void countClasses(
    const std::vector<std::pair<float, int> >& pairs, size_t idx1, size_t idx2, std::vector<int>& counts) {
    counts.assign(2, 0);
...
bool Chessboard::checkChessboard(const cv::Mat& image, cv::Size patternSize) const {
    ...
    for (float threshLevel = blackLevel; threshLevel < whiteLevel && !result; threshLevel += 20.0f) {
        cv::threshold(white, thresh, threshLevel + blackWhiteGap, 255, cv::ThresholdTypes::THRESH_BINARY);
        ...
        cv::threshold(black, thresh, threshLevel, 255, cv::ThresholdTypes::THRESH_BINARY_INV);
