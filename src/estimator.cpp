...
bool Estimator::relativePose(Matrix3d& relative_R, Vector3d& relative_T, int& l) {
    ...
    for (int i = 0; i < WINDOW_SIZE; i++) {
        ...
        corres = f_manager.getCorresponding(i, WINDOW_SIZE);  //找到在窗口内一直被观测的匹配点对
        if (corres.size() > 20) {
...
void Estimator::double2vector() {
    ...
    if (relocalization_info) {
        ...
        relo_t = rot_diff * Vector3d(
                                relo_Pose[0] - para_Pose[0][0], relo_Pose[1] - para_Pose[0][1],
...
void Estimator::MargOldFrame() {
    ...
    shared_ptr<backend::VertexPose> vertexExt(new backend::VertexPose());
    {
        ...
        pose << para_Ex_Pose[0][0], para_Ex_Pose[0][1], para_Ex_Pose[0][2], para_Ex_Pose[0][3], para_Ex_Pose[0][4],
            para_Ex_Pose[0][5], para_Ex_Pose[0][6];
    ...
    for (int i = 0; i < WINDOW_SIZE + 1; i++) {
        ...
        Eigen::VectorXd pose(7);
        pose << para_Pose[i][0], para_Pose[i][1], para_Pose[i][2], para_Pose[i][3], para_Pose[i][4], para_Pose[i][5],
            para_Pose[i][6];
        ...
        Eigen::VectorXd vb(9);
        vb << para_SpeedBias[i][0], para_SpeedBias[i][1], para_SpeedBias[i][2], para_SpeedBias[i][3],
            para_SpeedBias[i][4], para_SpeedBias[i][5], para_SpeedBias[i][6], para_SpeedBias[i][7],
            para_SpeedBias[i][8];
...
void Estimator::problemSolve() {
    ...
    problem.Solve(10, myslam::backend::Problem::SolverType::DogLeg);
