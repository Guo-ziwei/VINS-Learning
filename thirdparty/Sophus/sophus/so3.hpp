...
class SO3 : public SO3Base<SO3<Scalar_, Options>> {
 public:
  ...
  SOPHUS_FUNC static Sophus::Matrix<Scalar, DoF, DoF> rightJacobian(
    Tangent const& omega,
    std::optional<Scalar> const& theta_o = std::nullopt) {
  using std::cos;
  using std::sin;
  using std::sqrt;
  Tangent const omega_ = -omega;
  return leftJacobian(omega_, theta_o);
}
  ...
  SOPHUS_FUNC static Sophus::Matrix<Scalar, DoF, DoF> rightJacobianInverse(
    Tangent const& omega,
    std::optional<Scalar> const& theta_o = std::nullopt) {
  using std::cos;
  using std::sin;
  ...
  if (theta_sq <
    ...
    V_inv = Matrix3<Scalar>::Identity() + Scalar(0.5) * Omega +
            (Scalar(1) / (theta_sq) -
             (Scalar(1) + cos(theta)) / 2 * theta * sin(theta)) * (Omega * Omega);
  }
  return V_inv;
}
